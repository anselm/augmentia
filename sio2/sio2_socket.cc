...
{
	SIO2socket *_SIO2socket = ( SIO2socket * )calloc( 1, sizeof( SIO2socket ) );
	
	_SIO2socket->sck = -1;

	_SIO2socket->buf = ( unsigned char * ) malloc( SIO2_SOCKET_BUFFER_SIZE );

	_SIO2socket->sck_addr.sin_family = AF_INET;
...
{
	sio2SocketDisconnect( _SIO2socket );

	free( _SIO2socket->buf );
...
unsigned char sio2SocketListen( SIO2socket	   *_SIO2socket,
								...
								unsigned short  _maxc )
{
	static socklen_t len = sizeof( struct sockaddr_in );

	sio2SocketDisconnect( _SIO2socket );
	
	_SIO2socket->sck = socket( AF_INET, SOCK_STREAM, IPPROTO_TCP );
	
	if( _SIO2socket->sck < 0 )
	{ return 0; }
	
	_SIO2socket->sck_addr.sin_addr.s_addr = htonl( INADDR_ANY );
	_SIO2socket->sck_addr.sin_port = htons( _port );	

	if( bind( _SIO2socket->sck,
			  ( struct sockaddr * )&_SIO2socket->sck_addr,
			  len ) < 0 )
	{ return 0; }
	
	
	if( listen( _SIO2socket->sck, _maxc ) < 0 )
	{ return 0; }
	
	sio2SocketSetOpt( _SIO2socket );
								 ...
								 unsigned short		_port )
{
	...
	inet_pton( AF_INET, _ip, &_SIO2socket->sck_addr.sin_addr );
	
	if(	connect( _SIO2socket->sck,
				 ( struct sockaddr * )&_SIO2socket->sck_addr,
				 sizeof( _SIO2socket->sck_addr )  ) < 0 )
	{
		sio2SocketDisconnect( _SIO2socket );
		return 0;
	}
...
{
	if( _SIO2socket->sck > -1 )
	{
		close( _SIO2socket->sck );	
		shutdown( _SIO2socket->sck, SHUT_RDWR ); 
...
unsigned char sio2SocketSend( SIO2socket *_SIO2socket, char *_buf )
{
	int len = sio2StringLen( _buf );
	
	if( send( _SIO2socket->sck, _buf, len, 0 ) != len )
	{ return 0; }
...
int sio2SocketReceive( SIO2socket *_SIO2socket )
{
	memset( &_SIO2socket->buf[ 0 ], 0, SIO2_SOCKET_BUFFER_SIZE );
	
	return recv(  _SIO2socket->sck,
			 	 &_SIO2socket->buf[ 0 ],
				 SIO2_SOCKET_BUFFER_SIZE,
				 0 );
}
...
int sio2SocketAccept( SIO2socket *_SIO2socket )
{
	...
	int sck = accept( _SIO2socket->sck,
					  ( struct sockaddr * )&_SIO2socket->sck_addr,
					  &len );
	return sck;
}
...
void sio2SocketSetOpt( SIO2socket *_SIO2socket )
{
	int r = 0,
		v = 1;

	r = setsockopt( _SIO2socket->sck,
					SOL_SOCKET,
					SO_REUSEADDR,
					(char *)&v, sizeof( v ) );
	...
	fcntl( _SIO2socket->sck, F_SETFL, O_NONBLOCK );
}
