...
SIO2mist *sio2MistInit( char *_name )
{
	SIO2mist *_SIO2mist = ( SIO2mist * ) calloc( 1, sizeof( SIO2mist ) );

	_SIO2mist->col = sio2Vec4Init();
...
SIO2mist *sio2MistFree( SIO2mist *_SIO2mist )
{
	_SIO2mist->col = sio2Vec4Free( _SIO2mist->col );
...
void sio2MistCreate( SIO2mist *_SIO2mist,
					 ...
					 float	   _cend )
{
	memcpy( _SIO2mist->col, _col, 16 );
	
	_SIO2mist->dens   = _dens;
	_SIO2mist->mode   = _mode;
	_SIO2mist->cstart = _cstart;
	_SIO2mist->cend   = _cend;
}


void sio2MistRender( SIO2mist *_SIO2mist )
{
	if( !sio2IsStateEnabled( sio2->_SIO2state->flags, SIO2_FOG ) )
	{
		sio2StateEnable( sio2->_SIO2state, SIO2_FOG ) ;
		
		glFogf ( GL_FOG_MODE   , _SIO2mist->mode         );
		glFogf ( GL_FOG_DENSITY, _SIO2mist->dens         );
		glFogf ( GL_FOG_START  , _SIO2mist->cstart       );
		glFogf ( GL_FOG_END    , _SIO2mist->cend	     );
		glFogfv( GL_FOG_COLOR  , ( float * )_SIO2mist->col );
	}
}
...
{
	sio2StateDisable( sio2->_SIO2state, SIO2_FOG );
}
