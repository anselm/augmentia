...
{
	SIO2thread *_SIO2thread = ( SIO2thread * ) calloc( 1, sizeof( SIO2thread ) );
...
{
	sio2ThreadStop( _SIO2thread );
	
	while( _SIO2thread->state != SIO2_STOP )
	{ sio2Sleep( SIO2_THREAD_TIMEOUT ); }
	
	pthread_join( _SIO2thread->thread, NULL );
...
void sio2ThreadCreate( SIO2thread 		  *_SIO2thread,
					   ...
					   int				   _priority )
{
	_SIO2thread->_SIO2threadcallback = _SIO2threadcallback;
	_SIO2thread->priority			 = _priority;
	_SIO2thread->userdata			 = _userdata;

	_SIO2thread->thread_hdl = pthread_create( &_SIO2thread->thread,
											  NULL,
											  sio2ThreadRun,
											  (void *)_SIO2thread );
											  
	while( _SIO2thread->state != SIO2_PAUSE )
	{ sio2Sleep( SIO2_THREAD_TIMEOUT ); }
}


void sio2ThreadPlay( SIO2thread *_SIO2thread )
{ _SIO2thread->state = SIO2_PLAY; }


void sio2ThreadPause( SIO2thread *_SIO2thread )
{ _SIO2thread->state = SIO2_PAUSE; }


void sio2ThreadStop( SIO2thread *_SIO2thread )
{ _SIO2thread->state = SIO2_STOP; }


void *sio2ThreadRun( void *_ptr )
{
	SIO2thread *_SIO2thread = ( SIO2thread * )_ptr;

	struct sched_param param;
	
	param.sched_priority = _SIO2thread->priority;
	pthread_setschedparam( _SIO2thread->thread, SCHED_RR, &param );	

	sio2ThreadPause( _SIO2thread );
	
	while( _SIO2thread->state )
	{
		if( _SIO2thread->state == SIO2_PLAY )
		{ _SIO2thread->_SIO2threadcallback( _SIO2thread->userdata ); }
