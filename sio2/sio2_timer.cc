...
SIO2timer *sio2TimerInit( char *_name )
{
	SIO2timer *_SIO2timer = ( SIO2timer * )calloc( 1, sizeof( SIO2timer ) );
	
	sio2StringCpy( _SIO2timer->name, _name );	
...
SIO2timer *sio2TimerFree( SIO2timer *_SIO2timer )
{
...
void sio2TimerCreate( SIO2timer		    *_SIO2timer,
					  SIO2window	    *_SIO2window,
					  SIO2timercallback *_SIO2timercallback,
					  unsigned int		 _interval )
{
	_SIO2timer->_SIO2window = _SIO2window;
	
	_SIO2timer->_SIO2timercallback = _SIO2timercallback;
	
	_SIO2timer->interval = _interval;
}


void sio2TimerPlay( SIO2timer  *_SIO2timer )
{
	if( _SIO2timer->state == SIO2_STOP )
	{ _SIO2timer->tstart = _SIO2timer->_SIO2window->curr_time; }
	else
	{ _SIO2timer->tstart = _SIO2timer->_SIO2window->curr_time - _SIO2timer->tpause; }
	
	_SIO2timer->state = SIO2_PLAY;
}


void sio2TimerPause( SIO2timer  *_SIO2timer )
{
	_SIO2timer->tpause = _SIO2timer->_SIO2window->curr_time - _SIO2timer->tstart;
	_SIO2timer->state  = SIO2_PAUSE;
}


void sio2TimerStop( SIO2timer *_SIO2timer )
{ _SIO2timer->state = SIO2_STOP; }


void sio2TimerEvaluate( SIO2timer  *_SIO2timer )
{
	if( _SIO2timer->state == SIO2_PLAY &&
		( _SIO2timer->_SIO2window->curr_time - _SIO2timer->tstart ) >= _SIO2timer->interval )
	{
		if( _SIO2timer->_SIO2timercallback )
		{ _SIO2timer->_SIO2timercallback( _SIO2timer ); }
